...
data_bound_tmp = data_bound %>%
  ...
  mutate(a_d = ceiling(right_current_distance) /188,
         b_d = ceiling(left_current_distance) /188,
         a_t = ceiling(right_days_remaining) / 91,
         b_t = ceiling(left_days_remaining) / 91,
...
for(source in 1:2){
  for(frame in 1:2){
    ...
    dataList = list(
      ...
      s1 = Exptdata$s1,
      s2 = Exptdata$s2,
      Nsubj1 = max(Exptdata$s1),
      Nsubj2 = max(Exptdata$s2),
      y = t(y),
      a_d0 = t(a_d0),
      b_d0 = t(b_d0),
      a_t0 = t(a_t0),
    ...
    stan_rdump(names(dataList),file=paste0('./data/clean/',sources[source],"_",goal_types[frame],'_rdump.R'))
  }
