...
stg3 = left_join(stg2,stg1) %>%
  ...
  mutate(max_stg = max(stg),
         ...
         direction_tmp = case_when(
           dot_at_max_stg < 0.33 ~ 1,
           dot_at_max_stg > 3 ~ 2,
           dot_at_max_stg >= 0.33 & dot_at_max_stg <= 3 ~ 3
         ),
...
stg = ggplot(stg3) +
  geom_line(aes(x=dot,y=stg,group=subject,colour=direction),alpha=0.5) +
  ...
  scale_x_continuous(trans='log10',limits=c(0.2,5)) +
