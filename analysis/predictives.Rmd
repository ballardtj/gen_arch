...
```{r echo=FALSE, warning=FALSE,  message=FALSE,results='asis',fig.align="center", fig.height=8, fig.width=10}
#setwd("..")
library(rstan)
library(tidyverse)
library(gridExtra)
...
for (source in c('obs')){#},'opt')){
    ...
    for (structure in c('hier')){
      ...
      pp_data=generate_pp(fit=fit,dataList=ap_dataList,Nsamp=100,version=paste(structure,model,sep='_' ))
      ap_pp_plot= generate_pp_plot(pp_data)
      
      #Avoidance
      load(paste0("data/derived/av_",source,"_",structure,"_",model,"_fit.RData"))
      pp_data=generate_pp(fit=fit,dataList=av_dataList,Nsamp=100,version=paste(structure,model,sep='_' ))
      av_pp_plot= generate_pp_plot(pp_data)
      
      #Posterior predictives
     pp_fig = grid.arrange(
          arrangeGrob(
            arrangeGrob(ap_pp_plot[[1]] + theme(axis.text.x = element_text(size=6)) ,
              top="Experiment 1, Approach Condition"),
            arrangeGrob(av_pp_plot[[1]]  + theme(axis.text.x = element_text(size=6)) ,
              top="Experiment 1, Avoidance Condition"),
            arrangeGrob(ap_pp_plot[[2]]  + theme(axis.text.x = element_text(size=6)) ,
              top="Experiment 2, Approach Condition"),
            arrangeGrob(av_pp_plot[[2]]  + theme(axis.text.x = element_text(size=6)) ,
              top="Experiment 2, Avoidance Condition"),
            nrow=4
          ),
     ...
     save(file=paste0("figures/predictives,",source,"_",structure,"_",model,".RData"),plot=grid.arrange(pp_fig),width=7,height=5)
    }
  }
}
