...
```{r,eval=FALSE, include=FALSE, r,echo=FALSE}
setwd("..")
library(rstan)
library(tidyverse)
library(gridExtra)
...
generate_pp=function(fit_psy,fit_opt,dataList,Nsamp){
  
  posts_psy=rstan::extract(fit_psy)
  #posts_obs=rstan:extract(fit_opt)
  Npost=length(posts_psy[[1]])
  samples=sample(x=1:Npost,size=Nsamp)
  
  #pb <- txtProgressBar(min = 0, max = Nsamp*dataList$Nsubj, style = 3)
  #ctr=0
  #pp_list=list()
  #for(i in 1:Nsamp){
  pp_data <- foreach(i = 1:Nsamp,.combine='rbind') %dopar% {
    ...
    for(subj in 1:dataList$Nsubj){
      ...
      p_a_logit=goal_sub(posts_psy$w1_mean[samples[i]],
                         posts_psy$w1_sd[samples[i]],
                         posts_psy$w1[samples[i],], 
                         posts_psy$w2_mean[samples[i]],
                         posts_psy$w2_sd[samples[i]],
                         posts_psy$w2[samples[i],], 
                         posts_psy$w3_mean[samples[i]],
                         posts_psy$w3_sd[samples[i]],
                         posts_psy$w3[samples[i],], 
                         posts_psy$delta[samples[i],],
                         posts_psy$tau[samples[i],] , 
                         posts_psy$alpha[samples[i],],
                         dataList$Nobs,
                         subj,
                         dataList$expt,
                         dataList$s1,
      ...
      pp_data_tmp = data.frame(y_pred_psy=rep(NA,dataList$Nobs[subj]),
                               y_obs=NA,
                               a_d0=NA,
                               b_d0=NA,
                               a_t0=NA,
      ...
      pp_data_tmp$y_pred_psy = as.vector(1/(1+exp(-p_a_logit)))
      ...
      #optimal model run on observed decision points
      # p_a_logit=goal_sub(posts_opt$w1_mean[samples[i]],
      #                    posts_opt$w1_sd[samples[i]],
      #                    posts_opt$w1[samples[i],], 
      #                    posts_opt$w2_mean[samples[i]],
      #                    posts_opt$w2_sd[samples[i]],
      #                    posts_opt$w2[samples[i],], 
      #                    posts_opt$w3_mean[samples[i]],
      #                    posts_opt$w3_sd[samples[i]],
      #                    posts_opt$w3[samples[i],], 
      #                    posts_opt$delta[samples[i],],
      #                    posts_opt$tau[samples[i],] , 
      #                    posts_opt$alpha[samples[i],],
      #                    dataList$Nobs,
      #                    subj,
      #                    dataList$expt,
      #                    dataList$s1,
      ...
      #                    dataList$b_tod)
      # 
      # pp_data_tmp$y_pred_opt = as.vector(1/(1+exp(-p_a_logit)))
      # 
      
      if(any(is.na(pp_data_tmp$y_pred_psy))){
        stop(paste("NAN!, subj =",subj,"sample =",samples[i]))
      }
...
pp_data=generate_pp(fit_psy=fit,fit_opt=fit,dataList=dataList,Nsamp=100)
