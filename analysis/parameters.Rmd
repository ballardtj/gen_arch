...
```{r obs-vs-opt-mean-parameters, eval=FALSE, fig.align="center", fig.height=8, fig.width=10, include=FALSE}
...
```{r echo=FALSE, warning=FALSE,message=FALSE,fig.align="center", fig.height=8, fig.width=10}
...
means=posts %>%
  filter(source=="obs",model=="space",parameter=="w1"|parameter=="delta") %>%
  select(subject,.draw,frame,parameter,value) %>%
  spread(key=parameter,value=value) %>%
  group_by(subject,frame) %>%
