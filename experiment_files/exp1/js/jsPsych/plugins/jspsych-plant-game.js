...
jsPsych['plant-game'] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
        ...
        function keydownCallback(keydownevent, canvas, ctx, scr) {
            ...
            if (scr.keyNeeded == 'response' && (keydownevent.keyCode === 65 | keydownevent.keyCode === 75)) {
            ...
            if (keydownevent.keyCode != 65 && keydownevent.keyCode != 75 && keydownevent.keyCode != 86) {
            ...
            if (keyupevent.keyCode === scr.keyDown) {
                ...
                if (keyupevent.keyCode === 86) {
                    ...
                    if (scr.trialType === 'practice' && scr.preTrialScreen === 2) {
                        ...
                        var h = canvas.height;
                        ctx.fillStyle = "white";
                        ctx.clearRect(.01 * w, .84 * h, .34 * w, .15 * h);
                        ctx.clearRect(.65 * w, .84 * h, .34 * w, .15 * h);
                        //ctx.clearRect(.25 * w, .73 * h, .5 * w, .26 * h);

                        ctx.strokeStyle = "gold";
                        ctx.strokeRect(.01 * w, .47 * h, .34 * w, .37 * h);
                        ctx.strokeRect(.65 * w, .47 * h, .34 * w, .37 * h);

                        ctx.fillStyle = "brown";
                        ctx.lineWidth = 3;
                        ctx.font = "28px Times New Roman";
                        ctx.fillText("Here are the average amounts that each crop", .18 * w, .87 * h, .32 * w);
                        ctx.fillText("will grow per week when they are irrigated.", .18 * w, .91 * h, .32 * w);
                        ctx.font = "20px Times New Roman";
                        ctx.fillText("Press the 'v' key to continue.", .18 * w, .97 * h, .3 * w);
        ...
        function drawDecisionScreen(canvas, ctx, scr) {
            ...
            //Draw text above response option boxed
            ctx.fillStyle = "black";
            ctx.font = "32px Times New Roman";
            //ctx.fillText("Which crop do you want to irrigate this week?", .5 * w, .75 * h, w);

            //Draw boxes
            ctx.strokeRect(.02 * w, .84 * h, .32 * w, .14 * h);
            ctx.strokeRect(.66 * w, .84 * h, .32 * w, .14 * h);

            //Draw text within boxes
            ctx.font = "28px Times New Roman Bold";
            ctx.fillStyle = "black";
            ctx.fillText("Press 'a' to irrigate", .18 * w, .87 * h, .32 * w);
            ctx.fillText("Press 'k' to irrigate", .82 * w, .87 * h, .32 * w);


            ctx.fillStyle = scr.leftCropColor;
            ctx.font = "64px Times New Roman Bold";
            ctx.fillText(scr.leftCropType, .18 * w, .93 * h, .32 * w);
            ctx.fillStyle = scr.rightCropColor;
            ctx.fillText(scr.rightCropType, .82 * w, .93 * h, .32 * w);
        ...
        function feedbackLeft(canvas, ctx, scr) {
            ...
            ctx.clearRect(.01 * w, .47 * h, .34 * w, .52 * h);
            ctx.clearRect(.65 * w, .47 * h, .34 * w, .52 * h);
            ctx.strokeRect(.36 * w, .02 * h, .1 * w, .96 * h);
            //ctx.clearRect(.01 * w, .75 * h, .98 * w, .24 * h);
            //ctx.clearRect(.4 * w, .73 * h, .2 * w, .02 * h);
        ...
        function feedbackRight(canvas, ctx, scr) {
            ...
            ctx.clearRect(.01 * w, .47 * h, .34 * w, .52 * h);
            ctx.clearRect(.65 * w, .47 * h, .34 * w, .52 * h);
            ctx.strokeRect(.54 * w, .02 * h, .1 * w, .96 * h);
            // ctx.clearRect(.01 * w, .75 * h, .98 * w, .24 * h);
            // ctx.clearRect(.4 * w, .73 * h, .2 * w, .02 * h);
        ...
        function drawGoalDetails(canvas, ctx, scr) {
            ...
            ctx.font = "56px Times New Roman Bold";
            ...
            ctx.fillText('Average Growth per Week of Irrigation', .18 * w, .5 * h, .30 * w);
            ...
            //Expected Growth
            ctx.font = "24px Times New Roman";
            ctx.fillText('Average Growth per Week of Irrigation', .82 * w, .5 * h, .30 * w);


            //ctx.fillText(params.stimuli, .5 * w, .75 * h, w);

            //ctx.fillText("TIM", .5 * w, .77 * h, w);
            //ctx.font = "56px Times New Roman Bold";
            //ctx.fillText(scr.rightWeeksRemaining + " weeks", .82 * w, .55 * h, .30 * w);

            //Append plant images

            var cieling = 240; //top of scale
            var niches = 12; //number of intervals
            var topOfFrame = 0.02 * h
            var frameHeight = 0.96 * h

            var leftImageTopGap = ((cieling - scr.leftCurrentHeight) / cieling) * frameHeight;
            var leftImageHeight = (scr.leftCurrentHeight / cieling) * frameHeight;
            var rightImageTopGap = ((cieling - scr.rightCurrentHeight) / cieling) * frameHeight;
            var rightImageHeight = (scr.rightCurrentHeight / cieling) * frameHeight;
            ...
            for (i = 0; i < (niches + 1); i++) {
                ...
                ctx.fillText(i * cieling / niches, .5 * w, nicheY);
            };

            //Draw goal lines   
            ctx.fillStyle = scr.leftCropColor;
            ctx.fillRect(.36 * w, topOfFrame + ((cieling - scr.leftGoalHeight) / cieling) * frameHeight - 0.005 * h, .10 * w, .01 * h);
            ctx.fillStyle = scr.rightCropColor;
            ctx.fillRect(.54 * w, topOfFrame + ((cieling - scr.rightGoalHeight) / cieling) * frameHeight - 0.005 * h, .10 * w, .01 * h);


            //Draw Growth Per Week
            var scaleTop = 0.55 * h;
            var scaleHeight = 0.25 * h;

            //Left Axes
            ctx.moveTo(.11 * w, scaleTop + scaleHeight);
            ctx.lineTo(.25 * w, scaleTop + scaleHeight);
            ctx.stroke();

            ctx.moveTo(.12 * w, 0.55 * h);
            ctx.lineTo(.12 * w, scaleTop + scaleHeight + .02 * h);
            ctx.stroke();

            //Right Axes
            ctx.moveTo(.75 * w, scaleTop + scaleHeight);
            ctx.lineTo(.89 * w, scaleTop + scaleHeight);
            ctx.stroke();

            ctx.moveTo(.76 * w, 0.55 * h);
            ctx.lineTo(.76 * w, scaleTop + scaleHeight + .02 * h);
            ctx.stroke();

            //Determine Cieling and Niches
            var unitHeight = frameHeight / cieling;
            var nicheInt = 10;
            ...
            for (var i = 0; i < nicheCiel + 1; i += nicheInt) {
                ctx.fillStyle = scr.leftCropColor;
                ctx.moveTo(.11 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.lineTo(.13 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.stroke();
                ctx.fillText(i, .09 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);

                ctx.fillStyle = scr.rightCropColor;
                //ctx.moveTo(.89*w,scaleTop+scaleHeight - scaleHeight*i/nicheCiel  );
                //ctx.lineTo(.87*w,scaleTop+scaleHeight - scaleHeight*i/nicheCiel );
                //ctx.stroke();
                //ctx.fillText(i, .91 * w, scaleTop+scaleHeight - scaleHeight*i/nicheCiel );

                ctx.moveTo(.75 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.lineTo(.77 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.stroke();
                ctx.fillText(i, .73 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
            ...
            for (var i = nicheInt / 2; i < nicheCiel + 1; i += nicheInt) {
                ctx.fillStyle = scr.leftCropColor;
                ctx.moveTo(.115 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.lineTo(.125 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.stroke();

                ctx.fillStyle = scr.rightCropColor;
                // ctx.moveTo(.885*w,scaleTop+scaleHeight - scaleHeight*i/nicheCiel  );
                // ctx.lineTo(.875*w,scaleTop+scaleHeight - scaleHeight*i/nicheCiel );
                // ctx.stroke();

                ctx.moveTo(.755 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.lineTo(.765 * w, scaleTop + scaleHeight - scaleHeight * i / nicheCiel);
                ctx.stroke();
            }
            ...
            ctx.fillRect(.16 * w, scaleTop + scaleHeight - growthHeight, .04 * w, growthHeight);
            ctx.strokeRect(.16 * w, scaleTop + scaleHeight - growthHeight, .04 * w, growthHeight);

            ctx.fillStyle = scr.rightCropColor;
            growthHeight = scaleHeight * (scr.meanRightGrowthGivenRight / nicheCiel);
            ctx.fillRect(.8 * w, scaleTop + scaleHeight - growthHeight, .04 * w, growthHeight);
            ctx.strokeRect(.8 * w, scaleTop + scaleHeight - growthHeight, .04 * w, growthHeight);
